...
class ThemeTrack:
    ...
    def artist_text(self) -> str:
        return ", ".join(name for name in self.artist_names if name)

    @property
    def selection_label(self) -> str:
        label = self.theme_slug or self.display_theme
        title = self.song_title or self.display_theme
        text = f"{label} {title}".strip()
        if self.artist_text:
            return f"{text} by {self.artist_text}"
        return text
...
class AnimeThemesClient:
    ...
    async def get_theme_choices_for_slug(
        ...
        if not choices:
            raise AnimeThemesClientError("Nao achei faixas compativeis com esse filtro para esse anime.")
        return choices
        ...
        *,
        ...
        if not matches:
            raise AnimeNotFoundError(
                f'Nao encontrei "{cleaned}" no AnimeThemes. Tente um titulo mais completo.'
            )
    ...
    async def find_anime(self, query: str) -> dict[str, Any]:
        ...
        anime = await self._try_query_search(cleaned)
        if anime:
            return anime
    ...
    async def _try_query_search(self, query: str) -> dict[str, Any] | None:
        try:
            matches = await self.search_anime(query, limit=8)
        except AnimeThemesClientError:
            return None
        if not matches:
            return None
        return await self.get_anime_by_slug(matches[0].anime_slug)
    ...
    def _candidate_score(self, query: str, candidate: AnimeCandidate) -> int:
        ...
        score += min(candidate.matching_theme_count, 50)
        return score
    ...
    @staticmethod
    def _theme_display_sort_key(track: ThemeTrack) -> tuple[int, int, str]:
