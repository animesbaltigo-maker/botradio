...
def _selector_prompt_label(token: str) -> str:
    if token == KIND_OP:
        return "Escolha uma abertura"
    if token == KIND_ED:
        return "Escolha um encerramento"
    return "Escolha uma faixa"


def _selector_prompt_suffix(token: str) -> str:
    if token == KIND_OP:
        return "OP"
    if token == KIND_ED:
        return "ED"
...
def _build_tags_line(
    ...
    return " | ".join(part for part in parts if part) or "desconhecido"


def _build_info_footer(
    bot_username: str,
    info_link: str | None,
) -> str:
    info_html = (
        f'<a href="{escape(info_link, quote=True)}">Info</a>'
        if info_link
        else "Info"
    )
    username = bot_username.lstrip("@") if bot_username else "RadioAnimes_Bot"
    return f"@{escape(username)} | {info_html}"


def _build_usage_text(command_name: str) -> str:
    if command_name == "anime":
        return (
            "🎵 <b>Como usar o /anime</b>\n\n"
            "<blockquote>"
            "Envie o nome do anime para eu abrir a busca.\n"
            "Exemplo: /anime one piece\n"
            "Resultado: voce escolhe o anime e depois a faixa."
            "</blockquote>\n\n"
            "📻 Quer ouvir algo na hora? Use /radio."
        )
    if command_name == "op":
        return (
            "🎵 <b>Como usar o /op</b>\n\n"
            "<blockquote>"
            "Sem nome: toca uma abertura aleatoria.\n"
            "Com nome: abre a busca do anime.\n"
            "Exemplo: /op naruto"
            "</blockquote>"
        )
    if command_name == "ed":
        return (
            "🎵 <b>Como usar o /ed</b>\n\n"
            "<blockquote>"
            "Sem nome: toca um encerramento aleatorio.\n"
            "Com nome: abre a busca do anime.\n"
            "Exemplo: /ed bleach"
            "</blockquote>"
        )
    return (
        "🎵 <b>Comando</b>\n\n"
        "<blockquote>"
        "Envie o nome do anime para pesquisar."
        "</blockquote>"
    )
...
def _build_caption(track: ThemeTrack, bot_username: str) -> str:
    tags_line = _build_tags_line(track.year, track.season, track.media_format)
    ...
    return (
        ...
        f"</blockquote>\n\n"
        f"{_build_info_footer(bot_username, track.info_link)}"
    )
...
def _build_search_results_text(query: str, kind_token: str, bot_username: str) -> str:
    return (
        "🔎 <b>Escolha o anime</b>\n\n"
        f"<blockquote>"
        f"Busca: {escape(query)}\n"
        f"Filtro: {escape(_kind_short_label(kind_token))}\n"
        "Resultado: toque no anime certo abaixo."
        f"</blockquote>\n\n"
        f"@{escape(bot_username.lstrip('@') or 'RadioAnimes_Bot')} | AnimeThemes"
    )


def _build_theme_selector_caption(
    candidate: AnimeCandidate,
    kind_token: str,
    track_count: int,
    bot_username: str,
) -> str:
    tags_line = _build_tags_line(candidate.year, candidate.season, candidate.media_format)
    source_line = f"AnimeThemes | {track_count} opcoes"
    return (
        f"🗂️ <b>{escape(candidate.anime_name)}</b>\n\n"
        f"<blockquote>"
        f"Tema: {escape(_selector_prompt_label(kind_token))} ({escape(_selector_prompt_suffix(kind_token))})\n"
        f"Tags: {escape(tags_line)}\n"
        f"Fonte: {escape(source_line)}"
        f"</blockquote>\n\n"
        f"{_build_info_footer(bot_username, candidate.info_link)}"
    )
...
async def _send_anime_results_message(
    ...
    sent = await message.reply_text(
        _build_search_results_text(query_text, kind_token, _bot_username(context)),
        parse_mode=ParseMode.HTML,
...
async def _send_theme_selector_message(
    ...
    caption = _build_theme_selector_caption(
        candidate,
        kind_token,
        len(tracks),
...
async def anime_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not query:
        await message.reply_text(_build_usage_text("anime"), parse_mode=ParseMode.HTML)
        return

    await _play_by_kind(update, context, KIND_ANY)
