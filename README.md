...
- `/broadcast`: painel administrativo de transmissão em massa
...
- `FFMPEG_PATH`: caminho opcional do `ffmpeg`
- `MEDIA_CACHE_DIR`: diretório opcional de cache de mídia
- `PERSISTENCE_FILE`: arquivo opcional da persistência do bot
...
- `APP_CONCURRENT_UPDATES`: número de updates processados em paralelo
- `APP_CONNECTION_POOL_SIZE`: pool de conexões da API do Telegram
- `APP_GET_UPDATES_CONNECTION_POOL_SIZE`: pool específico do polling
- `APP_POOL_TIMEOUT`: timeout do pool de conexões
- `ADMIN_IDS`: IDs numéricos dos admins separados por vírgula
- `USER_REGISTRY_FILE`: arquivo opcional do cadastro persistente de usuários

## Observações

- O bot usa persistência local com `PicklePersistence` para salvar idioma e ação pendente por usuário.
- O broadcast usa um cadastro local de usuários para envio em massa e limpeza automática de usuários inválidos.
- O pipeline de mídia mantém cache em disco de áudio, capa e miniatura.
- O arquivo `.env` é carregado automaticamente no startup.
- Para usar o rate limiter dedicado do PTB, instale também as dependências de `requirements.txt`.
